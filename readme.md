...
- ВМ Ubuntu/Debian, от 8 ГБ RAM (тестировалось на VirtualBox, `ubuntu-22.04.5-desktop-amd64`, сетевой адаптер в режиме **Bridge** к основному интерфейсу хоста), не менее 40 ГБ памяти на диске
...
| Блокировка раскатки при непройденных тестах | CI-гейт (GitHub Actions) - ok|
...
На текущий момент может наблюдаться проблема сбора метрик в Grafana в дашборде Kyverno (сбор части метрик)


Текущее решение в ВМ:
```
